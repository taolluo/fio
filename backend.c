...
static long long usec_for_io(struct thread_data *td, enum fio_ddir ddir, uint64_t io_u_start)
{
	...
	if (td->o.rate_process == RATE_PROCESS_POISSON) {
        ...
        if (bytes >= low_half_bytes){ // in pulse fixme min=0
            ...
            ret = td->rate_next_period_arrival_time -  (td->o.square_wave_pulse_width -  usecs_in_pulse );
            ...
            ret = td->rate_next_period_arrival_time - (td->rate_this_period -  usecs_in_low_half);
            dprint(FD_RATE, "SW: linear, square wave bursting poisson next_io_time: %d\n", ret);
            return ret;
        }
...
static void *thread_main(void *data)
{
	...
	bool clear_state;
	int ret;
    dprint(FD_IO, "segfault2 call thread main \n");
    ...
    dprint(FD_IO, "segfault2 call post_init in thread_main\n");
	...
	if (rate_submit_init(td, sk_out)) // workqueue init for io_mode_offload
		goto err;
